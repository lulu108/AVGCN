...
class DepressionFaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if raw.ndim == 2:
            # 处理 D-vlog 的 (T, 136) 情况
            raw = raw.reshape(-1, 68, 2)
        ...
        regions = self.partition_regions(landmarks)
    ...
    def normalize_face(self, landmarks, skip_geometry=False):
        # skip_geometry=True 时跳过几何归一化步骤（中心化 + 面宽缩放）
        # 用于 DVLOG 路径：上游已用 IOD 做过归一化，避免双重缩放导致尺度体系矛盾
        if skip_geometry:
            # 直接使用传入的已归一化坐标
            normalized = landmarks.copy()
        else:
            # 1. 基础几何归一化 (解决相机远近/位置问题) —— LMVD 路径使用
