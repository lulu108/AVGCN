...
class MultiModalDataLoader(Dataset):
    ...
    def __init__(self, file_list, video_feature_path, audio_feature_path, 
                 ...
                 uniform_ratio=0.7, dvlog_aug_noise_std=0.005, dvlog_audio_mask_ratio=0.00,
                 ...
                 np_load_mmap=False,
                 lmvd_face_region_cache_dir=None,
        ...
        self.enable_feature_cache = bool(enable_feature_cache)
        self.feature_cache_size = int(max(1, feature_cache_size))
        self.np_load_mmap = bool(np_load_mmap)
        self.lmvd_face_cache_readonly = bool(lmvd_face_cache_readonly)
        self._segment_debug_printed = False

        # 每个 worker 持有独立 LRU，避免频繁重复读盘。
        self._video_cache = OrderedDict()
        self._audio_cache = OrderedDict()
        self._face_cache = OrderedDict()

        if lmvd_face_region_cache_dir is None:
            lmvd_face_region_cache_dir = os.path.join(
                self.face_landmark_path,
                f"_regions_cache_{self.region_partition_scheme}"
            )
        self.lmvd_face_region_cache_dir = lmvd_face_region_cache_dir

        if self.dataset == 'LMVD' and not self.lmvd_face_cache_readonly:
            os.makedirs(self.lmvd_face_region_cache_dir, exist_ok=True)
        ...
        print(
            f"[MultiModalDataLoader] cache: enable={self.enable_feature_cache}, "
            f"size={self.feature_cache_size}, mmap={self.np_load_mmap}"
        )
        if self.dataset == 'LMVD':
            print(
                f"[MultiModalDataLoader] LMVD face region cache dir: {self.lmvd_face_region_cache_dir} "
                f"(readonly={self.lmvd_face_cache_readonly})"
            )
        
    def __len__(self):
        return len(self.file_list)

    def _lru_get(self, cache_dict, key):
        if key not in cache_dict:
            return None
        val = cache_dict.pop(key)
        cache_dict[key] = val
        return val

    def _lru_put(self, cache_dict, key, value):
        if key in cache_dict:
            cache_dict.pop(key)
        elif len(cache_dict) >= self.feature_cache_size:
            cache_dict.popitem(last=False)
        cache_dict[key] = value

    def _get_cached_npy(self, path, cache_dict):
        """样本级 LRU：命中返回 copy，未命中读盘并写入缓存。"""
        if self.enable_feature_cache:
            hit = self._lru_get(cache_dict, path)
            if hit is not None:
                return hit.copy()

        load_kwargs = {}
        if self.np_load_mmap:
        ...
        if self.enable_feature_cache:
            self._lru_put(cache_dict, path, arr)

        return arr.copy()

    def _lmvd_face_cache_file(self, file_root):
        return os.path.join(
            self.lmvd_face_region_cache_dir,
            f"{file_root}_regions_{self.region_partition_scheme}.npz"
        )

    def _clone_face_regions(self, face_regions):
        return {k: v.copy() for k, v in face_regions.items()}

    def _load_lmvd_face_regions_precomputed(self, file_root):
        cache_file = self._lmvd_face_cache_file(file_root)

        if self.enable_feature_cache:
            hit = self._lru_get(self._face_cache, cache_file)
            if hit is not None:
                regions, actual_len, q_g, face_valid_eff_mean = hit
                return self._clone_face_regions(regions), int(actual_len), float(q_g), float(face_valid_eff_mean)
        ...
        regions = {
            k[len('region__'):]: np.asarray(data[k])
            for k in data.files
            if k.startswith('region__')
        }
        actual_len = int(data['actual_len']) if 'actual_len' in data else self.T_target
        q_g = float(data['q_g']) if 'q_g' in data else 1.0
        face_valid_eff_mean = float(data['face_valid_eff_mean']) if 'face_valid_eff_mean' in data else q_g

        if self.enable_feature_cache:
            self._lru_put(
                self._face_cache,
                cache_file,
                (regions, actual_len, q_g, face_valid_eff_mean),
            )

        return self._clone_face_regions(regions), actual_len, q_g, face_valid_eff_mean

    def _save_lmvd_face_regions_precomputed(self, file_root, face_regions, actual_len, q_g, face_valid_eff_mean):
        if self.lmvd_face_cache_readonly:
            return
        ...
        if os.path.exists(cache_file):
            return
        payload = {
            'actual_len': np.int64(actual_len),
            'q_g': np.float32(q_g),
            'face_valid_eff_mean': np.float32(face_valid_eff_mean),
        }
        for rk, rv in face_regions.items():
            payload[f'region__{rk}'] = np.asarray(rv)

        os.makedirs(os.path.dirname(cache_file), exist_ok=True)
        # 临时文件显式以 .npz 结尾，避免 np.savez_compressed 隐式补后缀后与 os.replace 源路径不一致。
        tmp_file = f"{cache_file}.tmp.{os.getpid()}.npz"
        alt_tmp_file = tmp_file + ".npz"
        np.savez_compressed(tmp_file, **payload)

        # 兼容极端环境下的隐式补后缀行为：优先替换真实存在的临时文件。
        if os.path.exists(tmp_file):
            src_tmp = tmp_file
        elif os.path.exists(alt_tmp_file):
            src_tmp = alt_tmp_file
        else:
            raise FileNotFoundError(
                f"LMVD face cache temp file missing after save: {tmp_file}"
            )

        # Windows 上也可用 os.replace 做原子替换，避免多 worker 写同一路径时破坏文件。
        os.replace(src_tmp, cache_file)

        # 最佳努力清理遗留临时文件。
        if src_tmp != tmp_file and os.path.exists(tmp_file):
            try:
                os.remove(tmp_file)
            except OSError:
                pass
        if src_tmp != alt_tmp_file and os.path.exists(alt_tmp_file):
            try:
                os.remove(alt_tmp_file)
            except OSError:
                pass
        ...
        if self.enable_feature_cache:
            self._lru_put(
                self._face_cache,
                cache_file,
                (self._clone_face_regions(face_regions), int(actual_len), float(q_g), float(face_valid_eff_mean)),
            )
    ...
    def __getitem__(self, idx):
        ...
        input_video = self._get_cached_npy(video_file, self._video_cache)
        input_audio = self._get_cached_npy(audio_file, self._audio_cache)
        ...
        if self.dataset == 'DVLOG':
            ...
            cached_face = self._load_lmvd_face_regions_precomputed(file_root)
            if cached_face is not None:
                face_regions, actual_len, q_g, face_valid_eff_mean = cached_face
            else:
                ...
                self._save_lmvd_face_regions_precomputed(
                    file_root=file_root,
                    face_regions=face_regions,
                    actual_len=actual_len,
                    q_g=q_g,
                    face_valid_eff_mean=face_valid_eff_mean,
                )

            # LMVD 默认视频质量全开（后续可扩展真实质量估计）
            q_v = 1.0
