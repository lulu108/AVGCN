...
class AnatomicalGCN(nn.Module):
    ...
    def __init__(self,
        ...
        if self.gcn_temporal_mode not in {'transformer', 'meanmax', 'meanmaxdiff', 'tcn'}:
            raise ValueError(
                f"Unknown gcn_temporal_mode: {self.gcn_temporal_mode!r}. "
                "Valid: ['transformer', 'meanmax', 'meanmaxdiff', 'tcn']"
            )
        ...
        # Region weight debug (enable from trainer)
        self.debug_region_weights = False
        self.debug_region_every = 200
        self._debug_region_step = 0
    ...
    def _forward_backbone_features(self, region_dict, actual_lens):
        ...
        self._maybe_log_region_weights(region_prior, sample_gate, final_region_weight)
        ...
        elif self.gcn_temporal_mode == 'meanmaxdiff':
            temporal_out = per_frame
            video_repr = self._masked_meanmaxdiff_project(temporal_out, time_mask)
        else:  # 'tcn'
    ...
    def _masked_meanmaxdiff_project(self, temporal_out, time_mask):
        valid_mask = time_mask.unsqueeze(-1).float()
        mean_feat = (temporal_out * valid_mask).sum(dim=1) / valid_mask.sum(dim=1).clamp(min=1.0)
        max_feat = temporal_out.masked_fill(~time_mask.unsqueeze(-1), -1e9).amax(dim=1)
        if temporal_out.size(1) <= 1:
            mad_feat = torch.zeros_like(mean_feat)
        else:
            diff = (temporal_out[:, 1:] - temporal_out[:, :-1]).abs()
            diff_mask = time_mask[:, 1:].unsqueeze(-1).float()
            mad_feat = (diff * diff_mask).sum(dim=1) / diff_mask.sum(dim=1).clamp(min=1.0)
        pooled = torch.cat([mean_feat, max_feat, mad_feat], dim=-1)
        return self.meanmaxdiff_proj(pooled)

    def _maybe_log_region_weights(self, region_prior, sample_gate, final_region_weight):
        if (not self.debug_region_weights) or (not self.training):
            return
        self._debug_region_step += 1
        if self._debug_region_step % int(max(self.debug_region_every, 1)) != 0:
            return
        with torch.no_grad():
            # Shapes: region_prior (1,1,R,1), sample_gate (B,1,R,1), final_region_weight (B,1,R,1)
            rp = region_prior.squeeze(0).squeeze(0).squeeze(-1)  # (R,)
            fw = final_region_weight.squeeze(1).squeeze(-1)      # (B, R)
            fw_mean = fw.mean(dim=0)
            fw_std = fw.std(dim=0, unbiased=False)
            fw_share = fw_mean / fw_mean.sum().clamp(min=1e-6)
            if sample_gate is not None:
                sg = sample_gate.squeeze(1).squeeze(-1)          # (B, R)
                sg_mean = sg.mean(dim=0)
                sg_std = sg.std(dim=0, unbiased=False)
                sg_min = sg.min(dim=0).values
                sg_max = sg.max(dim=0).values
            else:
                sg_mean = sg_std = sg_min = sg_max = None

            def _fmt(vals):
                return " ".join(f"{n}={v:.3f}" for n, v in zip(self.region_names, vals.tolist()))

            print(f"[RegionWeight][step={self._debug_region_step}] prior: {_fmt(rp)}")
            if sg_mean is not None:
                print(f"[RegionWeight][step={self._debug_region_step}] gate_mean: {_fmt(sg_mean)}")
                print(f"[RegionWeight][step={self._debug_region_step}] gate_std:  {_fmt(sg_std)}")
                print(f"[RegionWeight][step={self._debug_region_step}] gate_min:  {_fmt(sg_min)}")
                print(f"[RegionWeight][step={self._debug_region_step}] gate_max:  {_fmt(sg_max)}")
            print(f"[RegionWeight][step={self._debug_region_step}] final_mean: {_fmt(fw_mean)}")
            print(f"[RegionWeight][step={self._debug_region_step}] final_std:  {_fmt(fw_std)}")
            print(f"[RegionWeight][step={self._debug_region_step}] final_share:{_fmt(fw_share)}")
