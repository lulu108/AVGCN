...
class ViT(nn.Module):
    def __init__(self, spectra_size, patch_size, num_classes, dim, depth, heads, dim_mlp,
        ...
        # ViT 前端局部时序建模开关（Conv1d kernel=1 -> kernel=K）。
        ...
        # 轻量浅层音频 TCN 编码器（两层 k=3 + GELU + 残差），保持 (B,T,C) 维度不变。
        # 启用时将 proj_audio 的 kernel 固定为 1，避免与时序卷积功能重复叠加。
        ...
        # 与 temporal stem 解耦：audio_tcn 是独立编码器开关，可单独在 LMVD/DVLOG 启用。
        # 仅复用 stem_for 作为“是否作用于音频分支”的范围选择。
        audio_tcn_enabled = audio_tcn_enabled and (stem_for in {'audio', 'both'})
        audio_k = 1 if audio_tcn_enabled else (stem_k if (stem_enabled and stem_for in {'audio', 'both'}) else 1)
        video_p = video_k // 2
        audio_p = audio_k // 2
        ...
        self.proj_video = nn.Sequential(
            nn.Conv1d(
                in_channels=self.video_dim,
                out_channels=D_PROJECTION,
                kernel_size=video_k,
                stride=1,
                padding=video_p,
            ),
    ...
    def forward(self, X1, X2, mask=None, return_repr=False):
        # X1: (B, 915, 171)
        # X2: (B, 915, 128)
        ...
        X2 = self.audio_tcn_encoder(X2)
        X2_proj = self.proj_audio(X2)
        # 轻量音频全局表示（不额外引入音频 Transformer）
        audio_repr = self._temporal_global_pool(X2_proj)  # (B, D_PROJECTION)
        ...
        for blk in self.transformers:
            X = blk(X, mask=mask)
        vit_repr = X[:, 0]
        logits = self.classifier(vit_repr)

        # 兼容旧调用：默认仅返回 logits；按需返回中间表示。
        if return_repr:
            return logits, vit_repr, audio_repr
        return logits
...
class ViT_GCN_Fusion(nn.Module):
    ...
    def __init__(self, 
                 ...
                 tcn_kernel_size=3):  # 'late' | 'it_cross' | 'it_bi_gate' | 'concat' | 'afi'
        """
        Args:
            video_dim:   视频特征维度 (LMVD: 171, D-Vlog: 136)
            audio_dim:   音频特征维度 (LMVD: 128, D-Vlog: 25)
    ...
    def forward(self, video_features, audio_features, face_regions, actual_lens, quality=None):
        ...
        X2 = self.vit_branch.audio_tcn_encoder(X2)    # (B, audio_dim, T)
        X2_proj = self.vit_branch.proj_audio(X2)      # (B, 128, T)
        # ViT 前端音频全局表示（轻量、稳定、时间对齐要求低）
        audio_repr = self.vit_branch._temporal_global_pool(X2_proj)  # (B, 128)
        ...
        if self.fusion_mode == 'late':
            ...
            if self.temporal_attn_pool_effective:
                temporal_tokens = vit_transformed[:, 1:, :]  # (B, T_p, dim)
        ...
        return {
            ...
            'audio_repr': audio_repr, # (B, D_PROJECTION=dim//2)
        }
